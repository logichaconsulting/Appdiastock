...
public class ClosedInventory extends AppCompatActivity implements TaskDelegate, BaseActivityInterface, BaseFragment.OnFragmentInteractionListener {

    ActivityItem location = new ActivityItem(1, "Posizione", 23, TYPE_LOCATION, "", false, DataExchange.PROP_TOLOCATION, false);
    ActivityItem container = new ActivityItem(2, "Container", 20, TYPE_TEXT, "", false, DataExchange.PROP_CONTAINER, false);
    ActivityItem eanucc = new ActivityItem(3, "EAN/UCC", 200, TYPE_EANUCC, "", false, DataExchange.PROP_EANUCC, false);
    ...
    ActivityItem palletcode = new ActivityItem(12, "Codice Pallet", 3, TYPE_TEXT, "", false, DataExchange.PROP_PALLETCODE, false);
    ActivityItem serialnr = new ActivityItem(13, "Matricola", 50, TYPE_TEXT, "", false, DataExchange.PROP_SERIALNR, false);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        try {
            ...
            DataExchange.getInstance().setFunctionName(DataExchange.Operations.F3001); // CLOSED INVENTORY
            ...
            if (SetupFlags.getInstance().getSerialnrprefixes().length() > 0)
                prefixes = SetupFlags.getInstance().getSerialnrprefixes().split(Pattern.quote("|"));

            if (DataExchange.getInstance().getToLocation().getPositionCode() <= 0)
...
//
//                base.SetCurrentItem(location, DataExchange.getInstance().getToLocation().getEntirePosition(), false, false, false);
            ...
            if (base.GetCurrentItem() == location) {
                ...
                if (!DataFunctions.isNullOrEmpty(data)) {
                    ...
                    if (responseFromServer == 1) {
                        Presentation.ShowToLocation(base, DataExchange.getInstance().getToLocation(), false);
                ...
                if (responseFromServer == 0) {
                    ...
                    if (DataExchange.getInstance().getCurrentArticle() != null &&
                    ...
                    base.InitializeItem(qty, true, "Rettifica quantità");
                }
...
//                                                                                                                    Thread.currentThread().getStackTrace()[2].getMethodName());
                ...
                if (!DataFunctions.isNullOrEmpty(DataExchange.getInstance().getUnit()) &&
                        DataFunctions.isNullOrEmpty(DataExchange.getInstance().getPalletcode()) &&
                ...
                else if (SetupFlags.getInstance().getInventoryusesserialnr() &&
                        (DataExchange.getInstance().getCurrentArticle().getUsesserialnumber() == "O" ||
                                DataExchange.getInstance().getCurrentArticle().getUsesserialnumber() == "A"))
                    base.InitializeItem(serialnr);
                else
                    FinalStep();
                ...
                if (!DataFunctions.isNullOrEmpty(data)) {
                    ...
                    if (SetupFlags.getInstance().getInventoryusesserialnr() &&
                            (DataExchange.getInstance().getCurrentArticle().getUsesserialnumber() == "O" ||
                                    DataExchange.getInstance().getCurrentArticle().getUsesserialnumber() == "A"))
                        base.InitializeItem(serialnr);
                    else
            ...
            if (base.GetCurrentItem() == serialnr) {
                if (!DataFunctions.isNullOrEmpty(data)) {
                    if (data.indexOf('*') >= 0) {
                        AlertMessageBuilder alertMessageBuilder = new AlertMessageBuilder();
                        alertMessageBuilder.BuildDialog(getResources().getString(R.string.error), "Carattere * non valido. Rileggere il codice a barre!", AlertMessageBuilder.Severity.ERROR, this);
                        alertMessageBuilder.Show();
                        base.InitializeItem(serialnr, "");
                        return;
                    }
                    ...
                    if (prefixes != null && prefixes.length > 0) {
                        if (!DataFunctions.CheckSerialNrPrefix(data, prefixes)) {
                            AlertMessageBuilder alertMessageBuilder = new AlertMessageBuilder();
                            alertMessageBuilder.BuildDialog(getResources().getString(R.string.error), "Matricola con prefisso non valido!", AlertMessageBuilder.Severity.ERROR, this);
                            alertMessageBuilder.Show();
                            base.InitializeItem(serialnr, "");
                            return;
                        }
                    ...
                    if (!DataFunctions.CheckSerialNrLength(data, SetupFlags.getInstance().getSerialnrlength())) {
                        AlertMessageBuilder alertMessageBuilder = new AlertMessageBuilder();
                        alertMessageBuilder.BuildDialog(getResources().getString(R.string.error), "Matricola con lunghezza non valida!", AlertMessageBuilder.Severity.ERROR, this);
                        alertMessageBuilder.Show();
                        base.InitializeItem(serialnr, "");
                        return;
                    }
                    ...
                    if (shootedSerials == 0)
                        ...
                        for (String serial : serials) {
                            if (serial == data) {
                                AlertMessageBuilder alertMessageBuilder = new AlertMessageBuilder();
                                alertMessageBuilder.BuildDialog(getResources().getString(R.string.error), "Codice Già Letto", AlertMessageBuilder.Severity.ERROR, this);
                                alertMessageBuilder.Show();
                                base.InitializeItem(serialnr, "");
                                return;
                            }
                    ...
                    base.AddDisplayRow(data, "Ultima Matricola", false);
                    ...
                    if (shootedSerials < DataExchange.getInstance().getQty())
                        if (shootedSerials == 0)
                            ...
                            base.InitializeItem(serialnr, "", false, true);
                    else
                        FinalStep();
                } else
                    //BackToItem(ref serialnr);
                    FinalStep();
    ...
    private void FinalStep() {
        try {
            DataExchange.getInstance().setFunctionName(DataExchange.Operations.F3001);
    ...
    public void OnBackAction() {
        try {
            ...
            if (previousItem == null)
                ...
                InputArea inputArea = (InputArea) base.getActivity().findViewById(R.id.inputarea);

                inputArea.setBackground(base.getActivity().getDrawable(R.drawable.input_rounded_blue_border));
    ...
    public void CreateAction1() {
        if (base.GetCurrentItem() == qty) {
            try {
                InputArea inputArea = (InputArea) base.getActivity().findViewById(R.id.inputarea);

                if (DataExchange.getInstance().getGenericInt1() == 99) {
                    inputArea.setBackground(base.getActivity().getDrawable(R.drawable.input_rounded_blue_border));
                    DataExchange.getInstance().setGenericInt1(0);

                } else {
                    inputArea.setBackground(base.getActivity().getDrawable(R.drawable.input_rounded_red_border));
                    DataExchange.getInstance().setGenericInt1(99);
                }
            } catch (Exception e) {
                e.printStackTrace();
            }
...
//                if (methods[x].getName().contains(cloudConnector.getPostExceuteMethod()))
//                {
...
//                }
            ...
            if (result.equals("OK")) {
                ...
                WarehouseLocation location = DataExchange.getInstance().getToLocation();
                DataExchange.getInstance().format();
                DataExchange.getInstance().setToLocation(location);
                DataExchange.getInstance().setFunctionName(DataExchange.Operations.F3001); // CLOSED INVENTORY
                DataExchange.getInstance().setLoadCausal(menuItem.getDefLoadCausal());
